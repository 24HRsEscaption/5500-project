...
## NEU Quickstart instructions
Before beginning, install necessary dependencies (below).

1. Navigate to the hifigan folder and unzip the `generator_LJSpeech.pth.tar.zip` file. It should create a file called `generator_LJSpeech.pth.tar` afterwards.

2. Return to the `NEU-tts-2022/` directory and create the directory for the checkpoint 
```
...
```
gdown --fuzzy https://drive.google.com/file/d/1r3fYhnblBJ8hDKDSUDtidJ-BN-xAM9pe/view?usp=sharing
``` 

and put it in output/ckpt/LJSpeech. Unzip it. It should be the folder `LJSpeech_900000` containing the file `900000.pth.tar`. Move this file up one directory, so that its path `NEU-tts-2022/output/ckpt/LJSpeech/900000.pth.tar`.
...
This is where the resulting png and wav files will be created.
...
or with conda (for GPU hardware)
